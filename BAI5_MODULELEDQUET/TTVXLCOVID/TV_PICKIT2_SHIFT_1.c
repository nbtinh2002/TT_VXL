...
void xuat_8ledquet_2byte(int8 quet,int8 dulieu)
{
      spi_write2(quet);
      spi_write2(dulieu);
...
{
      unsigned int8 n;
      const unsigned int8 quetled[8]={0x7f,0xbf,0xdf,0xef,0xf7,0xfb,0xfd,0xfe};
      for(n=0;n<8;n++) 
      {
            xuat_8ledquet_2byte(quetled[n],led_7dq[n]);
            delay_ms(1);
            xuat_8ledquet_2byte(0xff,0xff);
      }
}
